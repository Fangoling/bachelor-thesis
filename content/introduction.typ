...
Leveraging Theia's modular architecture, the proposed Terminal Assistant Extension integrates with the existing ecosystem.

Building on the goal of improving terminal usability, this thesis introduces AI assistance for the terminal of the Theia IDE.
The implemented system focuses on automatic support for task-based executions in beginner-oriented workflows and on-demand support for direct terminal usage by more advanced students.
We organize the thesis objectives in these steps:
1. Establish a structured terminal state
2. Automatically summarize task executions
...
=== Automatically Summarize Task Executions
In the second stage, we implement the Terminal Assistant Core to automatically analyze task-based executions and visualize summaries.
The Output Analysis Agent generates a compact summary for build and debug tasks executed through Theia's task workflow.
...
In the next stage, we extend the Output Analysis Agent to automatically detect errors in the output. The system displays each detected error in a structured format, along with guidance for resolution. 
...
The thesis also defines a configuration setting in Theia IDE that exposes two controls: an "abstraction level" and an "assistance level".
...
These controls would allow the system to adjust how much information and guidance it exposes.

Low abstraction and assistance levels would support independent problem-solving, while higher levels would provide more direct support.

== Outline
Chapter 2 introduces the technical background on Theia, terminal emulation, shell integration, and the use of LLMs in development tools. Chapter 3 discusses related work and derives the research gap addressed by this thesis. Chapter 4 defines the system requirements and models the proposed solution from the user and domain perspective. Chapter 5 presents the system design, including the shell integration subsystem and the AI terminal assistant. Chapter 6 evaluates the usability of the implemented prototype. Chapter 7 concludes the thesis and outlines remaining limitations and future work.
