...
Although these tools provide valuable educational assistance, they typically rely on static artifacts, such as post-mortem log analysis, or require the student to context-switch to an external web portal. By operating outside the immediate execution loop, these systems disrupt the natural development workflow. We address this limitation by offering live, in-terminal intervention that integrates explanations directly alongside build and runtime output.

== Fragmented Terminal Output and Cognitive Overhead

The design of structured terminal interactions is further supported by software engineering literature concerning developer interruptions, context reconstruction, and fragmented information. Frequent context switching forces developers to repeatedly reconstruct their mental models of the system state, which degrades productivity and increases the risk of errors @abadTaskInterruptionSoftware2018. Presenting terminal output as a continuous, unstructured stream exacerbates this fragmented information problem, as users must manually filter noise to find relevant build and runtime output. 

While minimizing context switches is not the primary focus of this thesis, this literature still indirectly motivates structured command interactions. By isolating terminal output into discrete blocks and providing inline AI summaries, the proposed system may reduce the cognitive overhead associated with interpreting complex command-line output.

== Research Gap and Thesis Contribution

Existing research and industry practices demonstrate the technical viability of OSC-driven terminal integration, the professional utility of AI-assisted command-line tools, and the pedagogical need for automated error explanations. However, a significant gap remains at the intersection of these domains. Most widely deployed systems operate either as proprietary tools optimized for professionals or as detached pedagogical aids that interrupt the developer workflow. The existing work reviewed here shows that there is currently no open-source, extensible implementation that brings the standards of OSC-driven AI assistance directly into a pedagogical IDE workflow for learners.

To address this gap, we bridge the missing technical layers within the Eclipse Theia ecosystem. Compared to existing approaches, the core contribution is an integrated terminal assistant within Theia that combines summarization of monitored build and runtime output, structured error explanations, and configurable abstraction levels as a thesis objective. By establishing a structured terminal history for Theia as an enabling infrastructure, the thesis shows how a tightly integrated AI assistant can support students directly within their command-line workflow.
